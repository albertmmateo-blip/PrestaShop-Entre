-- Migration 001: Initial Loyalty System Database Schema
-- Description: Creates all tables, indexes, triggers, and functions for the loyalty points system
-- Created: 2026-02-13
-- Author: Development Environment Setup
...
CREATE INDEX IF NOT EXISTS idx_customers_prestashop_id ON customers(prestashop_customer_id) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_customers_email ON customers(prestashop_email) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_customers_phone ON customers(phone_number) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_customers_status ON customers(account_status) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_customers_email_hash ON customers(email_hash);
CREATE INDEX IF NOT EXISTS idx_customers_enrollment_date ON customers(enrollment_date DESC);
...
CREATE INDEX IF NOT EXISTS idx_cards_uid ON cards(card_uid) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_cards_number ON cards(card_number) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_cards_customer ON cards(customer_id) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_cards_status ON cards(card_status) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_cards_printing_batch ON cards(printing_batch);
...
-- Indexes for loyalty_accounts table
CREATE INDEX IF NOT EXISTS idx_loyalty_accounts_customer ON loyalty_accounts(customer_id) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_loyalty_accounts_balance ON loyalty_accounts(current_balance_points DESC) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_loyalty_accounts_last_activity ON loyalty_accounts(last_transaction_date DESC);
...
-- Indexes for loyalty_ledger table
CREATE INDEX IF NOT EXISTS idx_loyalty_ledger_customer ON loyalty_ledger(customer_id, ledger_sequence DESC);
CREATE INDEX IF NOT EXISTS idx_loyalty_ledger_account ON loyalty_ledger(account_id, transaction_date DESC);
CREATE INDEX IF NOT EXISTS idx_loyalty_ledger_transaction_date ON loyalty_ledger(transaction_date DESC);
CREATE INDEX IF NOT EXISTS idx_loyalty_ledger_transaction_type ON loyalty_ledger(transaction_type, transaction_date DESC);
CREATE INDEX IF NOT EXISTS idx_loyalty_ledger_source_reference ON loyalty_ledger(source_reference) WHERE source_reference IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_loyalty_ledger_order_id ON loyalty_ledger(order_id) WHERE order_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_loyalty_ledger_expiration ON loyalty_ledger(expiration_date) WHERE expiration_date IS NOT NULL AND expired_by_ledger_id IS NULL;
CREATE INDEX IF NOT EXISTS idx_loyalty_ledger_idempotency ON loyalty_ledger(idempotency_key) WHERE idempotency_key IS NOT NULL;
...
-- Indexes for order_mapping table
CREATE INDEX IF NOT EXISTS idx_order_mapping_customer ON order_mapping(customer_id, order_date DESC);
CREATE INDEX IF NOT EXISTS idx_order_mapping_pos_order ON order_mapping(pos_order_id) WHERE pos_order_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_order_mapping_prestashop_order ON order_mapping(prestashop_order_id) WHERE prestashop_order_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_order_mapping_earn_ledger ON order_mapping(earn_ledger_id) WHERE earn_ledger_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_order_mapping_order_date ON order_mapping(order_date DESC);
CREATE INDEX IF NOT EXISTS idx_order_mapping_sync_status ON order_mapping(sync_status) WHERE sync_status != 'synced';
...
CREATE INDEX IF NOT EXISTS idx_consent_customer ON consent_records(customer_id, consent_date DESC);
CREATE INDEX IF NOT EXISTS idx_consent_type_status ON consent_records(consent_type, consent_given, withdrawn);
CREATE INDEX IF NOT EXISTS idx_consent_withdrawal ON consent_records(withdrawal_date DESC) WHERE withdrawn = TRUE;
...
VALUES ('001', 'Initial loyalty system schema with all core tables')
